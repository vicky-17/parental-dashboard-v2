...
}, 30 * 60 * 1000); // Runs every 2 minutes (120,000 ms)
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        await user.save();
        console.log(`👤 [AUTH] ✅ New user registered: ${req.body.email}`);
...
app.post('/api/auth/login', async (req, res) => {
    ...
    if (!user) {
        console.log(`👤 [AUTH] ⚠️ Failed login attempt: Unknown email (${req.body.email})`);
    ...
    try {
        if (req.body.password === user.password) {
            ...
            console.log(`👤 [AUTH] ⚠️ Failed login attempt: Wrong password (${user.email})`);
...
app.delete('/api/devices/:deviceId', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`📱 [DEVICE] 🗑️ Deleted device and data: ${device.name} (${deviceId})`);
...
app.post('/api/devices/add', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`📱 [PAIRING] 🔑 Generated code: ${code} for user ${req.user.userId}`);
        res.json({ code });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/devices/pair', async (req, res) => {
    try {
        ...
        if (!pendingPairings[code]) {
            console.log(`📱 [PAIRING] ⚠️ Failed attempt: Invalid code (${code}) from device ${deviceId}`);
        ...
        console.log(`📱 [PAIRING] ✅ Success! Device paired: ${deviceName} (${deviceId})`);
...
cron.schedule('0 0 * * *', async () => {
    try {
        console.log(`⏱️ [CRON] 🌅 Starting daily app usage reset...`);
        await AppRule.updateMany({}, { $set: { usedToday: 0 } });
        console.log(`⏱️ [CRON] ✅ Daily reset complete.`);
    } catch (err) {
        console.error(`⏱️ [CRON] ❌ Daily reset failed:`, err);
    }
...
app.post('/api/apps', async (req, res) => {
    try {
        ...
        console.error("🔄 [SYNC] ❌ Error updating app usage:", error);
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/rules/update', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`🔒 [RULES] Updated rules for ${packageName} on device ${deviceId}`);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/browser-history', async (req, res) => {
    try {
        ...
        console.log(`🌐 [WEB] ✅ Saved ${newEntries.length} history items for ${deviceId}`);
        res.json({ success: true });
    } catch (error) {
        console.error("🌐 [WEB] ❌ Save History Error:", error);
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/web/update', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`🌐 [WEB] Config updated for ${deviceId}`);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/zones/add', authenticateToken, async (req, res) => {
    try {
        ...
        await newZone.save();
        console.log(`📍 [ZONE] ✅ Created new zone: ${name}`);
...
app.delete('/api/zones/:zoneId', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`📍 [ZONE] 🗑️ Deleted zone ${req.params.zoneId}`);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/location', async (req, res) => {
    try {
        ...
        for (const zone of zones) {
            ...
            if (isInside) {
                ...
                if (!lastAlert || lastAlert.timestamp < fiveMinsAgo) {
                    console.log(`📍 [GEOFENCE] 🚨 Breach detected for device ${deviceId} in zone: ${zone.name}`);
        ...
        console.error(`📍 [GEOFENCE] ❌ Error calculating location:`, error);
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/notifications/subscribe', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`🔔 [PUSH] ✅ User ${req.user.userId} subscribed to notifications.`);
...
async function sendNotificationToParent(userId, title, body) {
    try {
        ...
        if (!subRecord) {
            console.log(`🔔 [PUSH] ⚠️ No subscription found for user ${userId}`);
            return;
        }
        ...
        if (error.statusCode === 410) {
            ...
            console.log(`🔔 [PUSH] 🗑️ Deleted expired subscription for ${userId}`);
        }
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('register_pairing', (data) => {
        ...
        if (pendingPairings[code]) {
            ...
            console.log(`🔌 [SOCKET] Dashboard registered for pairing code: ${code}`);
        }
    });
    ...
    socket.on('delete_device', async (data) => {
        try {
            ...
            console.log(`🔌 [SOCKET] 🗑️ Device deleted via socket: ${device.name}`);
        } catch (error) {
